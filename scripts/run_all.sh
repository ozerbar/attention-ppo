...
FRAME_STACK=2 # Number of frames to stack in the observation

# WANDB logging flag
USE_WANDB=0  # set to 0 to disable wandb logging

# Use OBS_REPEAT, EXTRA_OBS_DIMS, and EXTRA_OBS_NOISE_STD in base env dir name
BASE_ENV_DIR="runs/${ENV_NAME}/${ENV_NAME}-x${OBS_REPEAT}-obs_noise_${OBS_NOISE}-extra_dims_${EXTRA_OBS_DIMS}-extra_std_${EXTRA_OBS_NOISE_STD}-frames_${FRAME_STACK}"
